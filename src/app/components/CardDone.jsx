...
const CategoryDropdown = dynamic(() =>
  import("../components/CategoryDropdown")
);
  ...
  useEffect(() => {
    import("../middleware/Profiles").then((module) => {
      ...
      const uniqueCategories = Array.from(
        new Set(module.default.map((profile) => profile.name))
      );
  ...
  const handleFilter = async (category) => {
    ...
    if (category === "All") {
      ...
      filteredData = usersData.filter(
        (user) =>
          user.name === category ||
          (user.gender === true && category === "LakiLaki") ||
          (user.gender === false && category === "Perempuan")
      );
    }
    ...
    for (let i = 0; i < filteredData.length; i++) {
      ...
      setFilteredProfiles((prevProfiles) => {
        const newProfile = {
          ...filteredData[i],
          uniqueKey: `${filteredData[i].id}_${category}`,
        };
        if (
          !prevProfiles.some(
            (profile) => profile.uniqueKey === newProfile.uniqueKey
          )
        ) {
  ...
  return (
    <div className="container mx-auto my-8">
      <div className="flex justify-between items-center mb-6">
        <h1
          data-aos="fade-down"
          className="  font-semibold hp:mr-4 ml-4 uppercase"
        >
          List Murid <p>X RPL2</p>
        </h1>
        <CategoryDropdown
          categories={["Semua Murid", "LakiLaki", "Perempuan"]}
